...
def __is_blacklisted_blockdev(dev_name):
    ...
    if os.path.exists("/sys/class/block/%s/device/model" %(dev_name,)):
        ...
        for bad in ("IBM *STMF KERNEL", "SCEI Flash-5", "DGC LUNZ"):
            if model.find(bad) != -1:
                ctx.logger.info("ignoring %s with model %s" %(dev_name, model))
                return True
...
def udev_device_get_zfcp_attribute(info, attr=None):
    ...
    if not attr:
        ctx.logger.debug("udev_device_get_zfcp_attribute() called with attr=None")
        return None
    ...
    if not os.path.isfile(attribute):
        ctx.logger.warning("%s is not a valid zfcp attribute" % (attribute,))
        return None
