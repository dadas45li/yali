...
def makeLoopDevice(device_name, file_name):
...
def removeLoopDevice(device_name, file_name):
...
class LibraryTestCase(unittest.TestCase):
    ...
    def setUp(self):
        for file in self._LOOP_DEVICES:
            dev = getFreeLoopDevice()
            makeLoopDevice(dev, file)
            self._loopMap[file] = dev

    def tearDown(self):
        for (file, dev) in self._loopMap.iteritems():
            removeLoopDevice(dev, file)
