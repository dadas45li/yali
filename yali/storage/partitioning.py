...
class LogicalVolumeRequest(Request):
    def __init__(self, lv):
        """ Create a LogicalVolumeRequest instance.

            Arguments:

                lv -- an LogicalVolume instance

        """
        super(LogicalVolumeRequest, self).__init__(lv)
...
class Chunk(object):
    ...
    def addRequest(self, req):
        ...
        ctx.logger.debug("adding request %d to chunk %s" % (req.device.id, self))
    ...
    def growRequests(self):
        ...
        for request in self.requests:
            ctx.logger.debug("request: %r" % request)
        ...
        while not self.done and self.pool and last_pool != self.pool:
            ...
            self.base = new_base
            ctx.logger.debug("%d requests and %d (%dMB) left in chunk" %
            ...
            for p in self.requests:
                ...
                ctx.logger.debug("new grow amount for request %d (%s) is %d "
                          "units, or %dMB" %
    ...
    def addRequest(self, req):
        ...
        if not self.requests:
            ...
            if chunk_end <= self.geometry.start:
                ...
                # cannot continue
                ctx.logger.error("chunk start sector is beyond disklabel maximum")
                raise PartitioningError("partitions allocated outside "
                                        "disklabel limits")

            new_pool = chunk_end - self.geometry.start + 1
            if new_pool != self.pool:
                ctx.logger.debug("adjusting pool to %d based on disklabel limits"
                            % new_pool)
                self.pool = new_pool
...
class VolumeGroupChunk(Chunk):
    ...
    def __init__(self, vg, requests=None):
        ...
        self.vg = vg
        self.path = vg.path
        super(VolumeGroupChunk, self).__init__(self.vg.size, requests=requests)

    def addRequest(self, req):
        """ Add a Request to this chunk. """
        if not isinstance(req, LogicalVolumeRequest):
            raise ValueError("VolumeGroupChunk requests must be of type "
                             "LogicalVolumeRequest")

        # round up growable requests to fill any used PE?
        super(VolumeGroupChunk, self).addRequest(req)
    ...
    for _part in new_partitions:
        ...
        ctx.logger.debug("allocating partition: %s ; id: %d ; disks: %s ;\n"
                        ...
                        (_part.name, _part.id, [d.name for d in req_disks],
                         boot, _part.req_primary,_part.req_size,
    ...
    for disk in disks:
        ...
        for chunk in chunks:
            ...
            for p in chunk.requests:
                ...
                ctx.logger.debug("partition %s (%d): %s" % (p.device.name,
                                                     p.device.id, ptype))
                ...
                ctx.logger.debug("new geometry for %s: %s" % (p.device.name,
                                                       new_geometry))
                start = end + 1
...
def growLVM(storage):
    ...
    for vg in storage.vgs:
        ...
        ctx.logger.debug("vg %s: %dMB free ; lvs: %s" % (vg.name, total_free,
                                                  [l.lvname for l in vg.lvs]))

        chunk = VolumeGroupChunk(vg, requests=[LogicalVolumeRequest(l) for l in vg.lvs])
        chunk.growRequests()
...
def logicalVolumeCompare(lv1, lv2):
    ...
    if lv1.req_grow and lv2.req_grow:
        if not lv1.req_max_size and lv2.req_max_size:
            ret -= 25
        elif lv1.req_ax_size and not lv2.req_max_size:
            ret += 25
        else:
            ret -= cmp(lv1.req_max_size, lv2.req_max_size) * 25
